...
ORDER = ("polydT", "splint", "5'TSO", "3'TSO")                 # panels, left to right
...
SHORT = {"splint": "splint", "polydT": "poly(dT)", "5'TSO": "TSO (5'DBCO)", "3'TSO": "TSO (3'DBCO)"}
...
def render(DATA, out):
    ...
    for pi, (ax, (name, recs)) in enumerate(zip(axes, DATA)):
        ...
        if name == "polydT":                              # band labels, left of the first panel
            ax.text(0.3, 3.85, "DRUG-seq benchmark (>96%)", ha="left", va="bottom",
                    fontsize=10, color=BAND_MID_INK, zorder=2)
            ax.text(0.3, 0.965, "bobcode benchmark (>99%)", ha="left", va="bottom",
                    fontsize=10, color=BAND_INK, zorder=2)
        ...
        ax.set_title(TITLE[name], fontsize=10.8, pad=5)
